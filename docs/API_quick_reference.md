...
詳細: [認証 API 詳細](detailed/authentication.md)
...
詳細: [題材・セクション API 詳細](detailed/subjects_sections.md)
...
- 最大サイズ: 5MB
- 画像が登録されている場合、レスポンスの `imageUrl` に S3 の URL が含まれます
...
詳細: [進捗管理 API 詳細](detailed/progress.md)
...
詳細: [プロフィール API 詳細](detailed/profile.md)
...
詳細: [管理者 API 詳細](detailed/admin.md)

### 開発者 API

| メソッド | パス                      | 認証                | リクエスト例 | レスポンス例                |
| -------- | ------------------------- | ------------------- | ------------ | --------------------------- |
| GET      | `/api/dev/verify`         | `X-Dev-Key`（任意） | -            | `{isDeveloper: true/false}` |
| GET      | `/api/dev/endpoints`      | `X-Dev-Key`         | -            | `{endpoints: [...], count}` |
| DELETE   | `/api/dev/users/{userId}` | `X-Dev-Key`         | -            | 204 No Content              |

**注意**: `ROLE_DEVELOPER`ユーザーは削除不可

詳細: [開発者 API 詳細](detailed/developer.md)
...
const updateSectionWithImage = async (
  ...
  const res = await fetch(`/api/subjects/${subjectId}/sections/${sectionId}`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${token}` },
    body: formData,
  });
