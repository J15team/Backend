...
class ProgressController(
    ...
    /** ユーザーの進捗状態を取得 ログイン後、フロントエンドの進捗バーに表示するためのデータを返す */
    @GetMapping("/{userId}")
    fun getUserProgress(@PathVariable userId: String): ResponseEntity<UserProgressResponse> {
        val userIdObj = UserId(UUID.fromString(userId))
        val userProgress = progressUseCase.getUserProgress(userIdObj)
        return ResponseEntity.ok(UserProgressResponse.from(userProgress))
    }

    /** セクション完了をマーク フロントエンドからセクション完了通知を受け取り、DBに記録 */
    @PostMapping("/{userId}/sections")
    fun markSectionAsCleared(
        ...
        val result = progressUseCase.markSectionAsCleared(userIdObj, sectionId)
    ...
    /** 特定セクションの完了状態をチェック */
    @GetMapping("/{userId}/sections/{sectionId}")
    fun checkSectionCleared(
        ...
        val isCleared = progressUseCase.isSectionCleared(userIdObj, sectionIdObj)
        return ResponseEntity.ok(mapOf("isCleared" to isCleared))
    }

    /** セクション完了を取り消し（デバッグ用） */
    @DeleteMapping("/{userId}/sections/{sectionId}")
    fun unmarkSectionAsCleared(
        ...
        val result = progressUseCase.unmarkSectionAsCleared(userIdObj, sectionIdObj)
