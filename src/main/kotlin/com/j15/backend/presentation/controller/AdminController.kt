...
/** 管理者専用コントローラ IP制限またはキー検証を通じてのみアクセス可能 */
@RestController
...
class AdminController(private val adminUseCase: AdminUseCase) {
    ...
    /**
     * 管理者アカウントを作成 POST /api/admin/users
     *
    ...
    /** 管理者ユーザー一覧を取得 GET /api/admin/users */
    @GetMapping("/users")
    ...
    /** 管理者ユーザーの詳細を取得 GET /api/admin/users/{userId} */
    @GetMapping("/users/{userId}")
    @PreAuthorize("hasRole('ADMIN')")
    fun getAdminUser(@PathVariable userId: String): ResponseEntity<Any> {
        return try {
    ...
    /** 管理者ユーザーを更新 PUT /api/admin/users/{userId} */
    ...
    /** 管理者ユーザーを削除 DELETE /api/admin/users/{userId} */
    @DeleteMapping("/users/{userId}")
    @PreAuthorize("hasRole('ADMIN')")
    fun deleteAdminUser(@PathVariable userId: String): ResponseEntity<Any> {
        return try {
    ...
    fun getAllUsers(): ResponseEntity<Any> {
        return try {
            ...
            val userResponses =
                    users.map { user ->
                        mapOf(
                                "userId" to user.userId.value.toString(),
            ...
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(mapOf("error" to "ユーザー一覧の取得中にエラーが発生しました"))
        }
    }
    ...
    fun deleteAllNonAdminUsers(): ResponseEntity<Any> {
        return try {
            ...
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(mapOf("error" to "一般ユーザーの削除中にエラーが発生しました"))
        }
    }
}
