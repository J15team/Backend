...
class AdminUseCase(
    ...
    /** 管理者ユーザー作成の結果 */
    data class AdminUserCreationResult(val user: User)
    ...
    fun updateAdminUser(
        ...
        val updatedPasswordHash =
                password?.let { PasswordHash(passwordEncoder.encode(it)) }
     ...
     * @return 削除されたユーザー数
     */
    fun deleteAllNonAdminUsers(): Int {
        val nonAdminUsers = userRepository.findAll().filter { it.role != UserRole.ROLE_ADMIN }

        nonAdminUsers.forEach { user -> userRepository.deleteById(user.userId) }

        logger.info("Deleted ${nonAdminUsers.size} non-admin users")
    ...
    fun getAllAdminUsers(): List<User> {
        return userRepository.findAll().filter { it.role == UserRole.ROLE_ADMIN }
    }
